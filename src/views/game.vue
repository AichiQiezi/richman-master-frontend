        ...
        <div id="middle">
          ...
          <div id="middle-middle">
            ...
            <div id="throw_dice" v-else-if="roomInfo.roomStatus === '等待' && roomInfo.owner === currentPlayer">
              ...
              <button @click="addBot()">添加人机</button>
            </div>
            <div id="throw_dice" v-else-if="roomInfo.roomStatus === '等待' && roomInfo.owner !== currentPlayer">
              请等待房主开始游戏
            </div>
        ...
        <div id="playerInfo">
          <table class="user">
            ...
            <tr v-for="(player, id) in players" :key="player._id">
              <td>
                <div v-if="id === currentRound" class="userColor" :style="'background-color:'+player.color"></div>
              </td>
              <td>
                <div class="userColor" :style="'background-color:'+player.color"></div>
              </td>
      ...
      <el-table :data="saleDialog.areas" max-height="400">
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="saleHouse(scope.$index, scope.row)">出售一级
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script src="../assets/js/socket.io.js">
</script>
  ...
  methods: {
    ...
    socketMessage() {
      ...
      this.sockets.subscribe(this.roomId + "-throwDiceBack", msg => {
        this.isBot = msg.isBot
        this.diceMsg = msg;
        this.currentRound = msg.nextPlayer;
        this.isNextPlayerBot = msg.isNextPlayerBot
      ...
      // this.sockets.subscribe(this.roomId + "-runBotBack", msg => {
      //   this.isBot = true
      //   console.log("收到 bot 信息")
      //   console.log(msg)
      //   this.diceMsg = msg;
      //   this.currentRound = msg.nextPlayer;
      //   this.isNextPlayerBot = msg.isNextPlayerBot
      ...
      //   }
      //
      //   console.log("下一个是否为机器人：" + this.isNextPlayerBot)
      //   console.log(this.currentPlayer === this.currentRound)
      //   if (this.currentPlayer === this.currentRound && this.isNextPlayerBot === true) {
      //     console.log("上一个事件是否完成：" + this.isPreviousFinished)
      //     this.$socket.emit("runBot", {
      //       token: localStorage.richman_token,
      //       roomId: this.roomId,
      //       isNextPlayerBot: this.isNextPlayerBot
      //     });
      //     this.isNextPlayerBot = false
      //   }
      // });
      ...
      setInterval(() => {
        console.log('当前用户id以及，前一个用户操作是否完成' + this.currentRound + "\t" + this.isPreviousFinished)
        if (this.isPreviousFinished) {
          if (this.currentRound.includes('bot')) {
            this.runBot()
          }
        }
      }, 5000)
    ...
    addBot() {
      if (this.roomInfo.userNum >= this.roomInfo.max) {
        Message({
          message: "人数已满，无法添加人机",
          type: "warning",
          duration: 5 * 1000
    ...
    throwDice() {
      ...
      this.isPreviousFinished = false
      this.diceButton = true;
    },

    runBot() {
      console.log("完成了，开始执行bot")
      this.$socket.emit("runBot", {
        playerId: this.currentRound,
        roomId: this.roomId,
      });
    },
      ...
      if (step === endStep) {
        ...
        if (type === "dice") {
          ...
          if (
            ...
            console.log("bottest:" + JSON.stringify(this.diceMsg))
            console.log(this.currentRound)
            if (this.diceMsg._id.includes('bot') && this.diceMsg.isBot) {
              console.log("发送bottest")
              this.$socket.emit("botTest", {
                token: localStorage.richman_token,
                roomId: this.roomId,
                currentPlayer: this.diceMsg._id,
              });
            }
